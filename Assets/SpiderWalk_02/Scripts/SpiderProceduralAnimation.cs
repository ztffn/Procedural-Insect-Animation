...
namespace MaxiPoney
{
    public class SpiderProceduralAnimation : MonoBehaviour
    {
        ...
        Vector3[] MatchToSurfaceFromAbove(Vector3 point, float halfRange, Vector3 up)
        {
            ...
            if (Physics.SphereCast(ray, sphereCastRadius, out hit, 6f * halfRange))
            {
                res[0] = hit.point;
                res[1] = hit.normal;
                Debug.DrawRay(hit.point, hit.normal, Color.blue, 6f * halfRange);
        ...
        void FixedUpdate()
        {
            ...
            for (int i = 0; i < nbLegs; ++i)
            {
                ...
                DebugShapes.DrawPoint(desiredPositions[i], Color.magenta, 0.1f);
                DebugShapes.DrawPoint(desiredPositions[i] + velocity * velocityMultiplier, Color.yellow, 0.1f);
            ...
            if (indexToMove != -1 && !legMoving[0])
            {
                ...
                {
                   /*
                    * //manual gait hack attempt though not working...
                    
                    if (indexToMove == 0 || indexToMove == 3 || indexToMove == 4)
                    { StartCoroutine(PerformStep(0, positionAndNormalBwd[0]));  
                        StartCoroutine(PerformStep(3, positionAndNormalBwd[0]));  
                        StartCoroutine(PerformStep(4, positionAndNormalBwd[0]));  
                    }
                    else if (indexToMove == 1 || indexToMove == 2 || indexToMove == 5)
                    { StartCoroutine(PerformStep(1, positionAndNormalBwd[0]));  
                        StartCoroutine(PerformStep(2, positionAndNormalBwd[0]));  
                        StartCoroutine(PerformStep(5, positionAndNormalBwd[0]));  
                    }
                   */
            ...
            if (nbLegs > 3 && bodyOrientation)
            {
                ...
                Vector3 v2 = legTargets[nbLegs-2].position - legTargets[nbLegs-1].position;
        ...
        private void OnDrawGizmos()
        {
            for (int i = 0; i < nbLegs; ++i)
            {
                ...
                Gizmos.DrawWireSphere(legTargets[i].position, 0.25f);
                Gizmos.color = Color.green;
                Gizmos.DrawWireSphere(transform.TransformPoint(defaultLegPositions[i]), stepSize);
                Gizmos.DrawSphere (lastpoint, sphereCastRadius);
            }
        }
    }
}