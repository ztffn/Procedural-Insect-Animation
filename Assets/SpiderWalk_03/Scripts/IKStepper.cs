...
namespace philippschofield
{
    ...
    public class IKStepper : MonoBehaviour
    {
        ...
        [Header("Debug")] public bool showDebug;
        public bool printDebugLogs;
        public bool pauseOnStep = false;
        [Range(1, 1000.0f)] public float debugIconScale;

        [Header("Step Layer")] public LayerMask stepLayer;

        [Header("Leg Synchronicity")] public IKStepper[] asyncChain;

        [Header("Step Timing")] [Range(0.0f, 5.0f)]
        public float stepCooldown = 0.0f;

        private float timeSinceLastStep;
        [Range(0.0f, 2.0f)] public float stopSteppingAfterSecondsStill;

        [Header("Step Transition")] [Range(0.0f, 10.0f)]
        public float stepHeight;

        public AnimationCurve stepAnimation;

        [Header("Default Position")] [Range(-1.0f, 1.0f)]
        public float defaultOffsetLength;

        [Range(-1.0f, 1.0f)] public float defaultOffsetHeight;
        [Range(-1.0f, 1.0f)] public float defaultOffsetStride;

        [Header("Default Overshoot Multiplier")] [Range(1.0f, 2.0f)]
        public float defaultOvershootMultiplier = 1.5f;

        [Header("Last Resort IK Target Position")] [Range(0f, 1.0f)]
        public float lastResortHeight;

        [Header("Ray Casting")] public CastMode castMode;
        public float radius;

        [Header("Frontal Ray")] [Range(0f, 1f)]
        public float rayFrontalHeight;

        [Range(0f, 1f)] public float rayFrontalLength;
        [Range(0f, 1f)] public float rayFrontalOriginOffset;

        [Header("Outwards Ray")] public Vector3 rayTopFocalPoint;
        [Range(0f, 1f)] public float rayOutwardsOriginOffset;
        [Range(0f, 1f)] public float rayOutwardsEndOffset;

        [Header("Downwards Ray")] [Range(0f, 6f)]
        public float downRayHeight;

        [Range(0f, 6f)] public float downRayDepth;

        [Header("Inwards Ray")] public Vector3 rayBottomFocalPoint;
        [Range(0f, 1f)] public float rayInwardsEndOffset;
        ...
        private void Awake()
        {
            ...
            lastResortPositionLocal = defaultPositionLocal +
                                      new Vector3(0, lastResortHeight * 4f * spider.getNonScaledColliderRadius(), 0);
        ...
        private Vector3 calculateDefault()
        {
            ...
            Vector3 pivot = spider.getColliderBottomPoint() +
                            Vector3.ProjectOnPlane(rootJoint.getRotationPoint() - spider.transform.position, normal);

            Vector3 midOrient = Quaternion.AngleAxis(0.5f * (rootJoint.maxAngle + rootJoint.minAngle), rootRotAxis) *
                                toEnd;

            //Set the following debug variables for the DOF Arc
            minOrient = spider.transform.InverseTransformDirection(
                Quaternion.AngleAxis(rootJoint.minAngle, rootRotAxis) * toEnd);
            maxOrient = spider.transform.InverseTransformDirection(
                Quaternion.AngleAxis(rootJoint.maxAngle, rootRotAxis) * toEnd);

            // Now set the default position using the given stride, length and height
            Vector3 defOrientation =
                Quaternion.AngleAxis(defaultOffsetStride * 0.5f * rootJoint.getAngleRange(), rootRotAxis) * midOrient;
            Vector3 def = pivot;
        ...
        private void updateCasts()
        {
            ...
            Vector3 frontalPredictionEnd = frontal +
                                           Vector3.ProjectOnPlane(prediction - frontal, spider.transform.up)
                                               .normalized * rayFrontalLength * chainLength;
            Vector3 frontalDefaultEnd = frontal +
                                        Vector3.ProjectOnPlane(defaultPos - frontal, spider.transform.up).normalized *
                                        rayFrontalLength * chainLength;
            ...
            Vector3 inwardsPredictionEndFar = Vector3.Lerp(prediction, bottom,
                inwardsPredictionLength / Vector3.Distance(prediction, bottom));

            Vector3 inwardsDefaultEndClose = bottomBorder;
            Vector3 inwardsDefaultEndMid = bottomMid;
            Vector3 inwardsDefaultEndFar = Vector3.Lerp(defaultPos, bottom,
                inwardsDefaultLength / Vector3.Distance(prediction, bottom));
         ...
         */
        private Cast getCast(Vector3 start, Vector3 end, float radius, Transform parentStart = null,
            Transform parentEnd = null)
        {
        ...
        */
        private IEnumerator Step(float stepTime)
        {
            ...
            if (ikChain.getTarget().grounded || newTarget.grounded)
            {
                ...
                while (time < stepTime)
                {
                    lerpTarget.position = Vector3.Lerp(lastTarget.position, newTarget.position, time / stepTime) +
                                          stepHeight * 0.01f * spider.getScale() *
                                          stepAnimation.Evaluate(time / stepTime) * spider.transform.up;
        ...
        private TargetInfo findTargetOnSurface()
        {
            ...
            if (Physics.OverlapSphere(rootJoint.getRotationPoint(), chainLength, layer,
            ...
            foreach (var cast in casts)
            {
                ...
                if (cast.Value.castRay(out hitInfo, layer))
                {
                    ...
                    if (cast.Key == "Frontal" &&
                        Vector3.Angle(cast.Value.getDirection(), hitInfo.normal) < 180f - 65f) continue;
         ...
         */
        private void drawDebug(bool points = true, bool steppingProcess = true, bool rayCasts = true,
            bool DOFArc = true)
        {
            ...
            if (DOFArc)
            {
                ...
                DebugShapes.DrawCircleSection(p, v, w, rootJoint.getRotationAxis(), minDistance, chainLength,
                    Color.red);
            }
        }
